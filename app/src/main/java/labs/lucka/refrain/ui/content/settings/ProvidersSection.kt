...
fun ProvidersSection(providers: List<String>, mutable: Boolean, isProviderEnabled: (provider: String) -> Boolean) {
    ...
    fun nameIdOf(provider: String): Int {
        return when (provider) {
            LocationManager.GPS_PROVIDER -> R.string.providers_gps
            LocationManager.FUSED_PROVIDER -> R.string.providers_fused
            LocationManager.NETWORK_PROVIDER -> R.string.providers_network
            LocationManager.PASSIVE_PROVIDER -> R.string.providers_passive
            else -> R.string.providers_undefined
        }
    }
    Section(stringResource(R.string.providers), modifier = Modifier.selectableGroup()) {
        for (provider in providers) {
            var text = stringResource(nameIdOf(provider))
            if (!isProviderEnabled(provider)) {
                text += stringResource(R.string.providers_disabled_suffix)
            }
